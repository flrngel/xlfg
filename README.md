...
**v6.5.10** reverts the v6.5.5+ surface that was causing phase-stopping regressions: the fenced "what to emit" question template (added v6.5.5) and the bloated fenced completion-summary template (expanded v6.5.5+) were competing with the dispatch instruction after each phase returned. Per Anthropic's agent-loop docs, when the model emits text without a tool call, `stop_reason` becomes `end_turn` and the harness exits the loop — which is exactly what was happening when the model wrote "Handing off to context phase." after a phase returned. v6.5.10 reverts both surfaces to v6.5.2's brevity (one-liner question gate, prose-described completion summary) and restores calm imperative voice (drop ALLCAPS NEVER/MUST). Conductor body shrinks 191 → 163 lines (back to v6.5.2 level). v6.5.4 handoff cues on all 9 phase bodies stay; the durable archive convention stays; the bullet-output style for the actual run summary stays. Tests reshaped for the prose-described format. Test surface 63 → 61.

**v6.5.9** fixes a phase-stopping regression that v6.5.8 only partially addressed. Symptom: phases were ending the turn after recall / intent / plan instead of dispatching the next phase. Cause: the v6.5.5 conductor body grew 28% (162 → 208 lines) and added a dedicated `## Question template` section that — even gated to `needs-user-answer` only — read as a competing fenced "what to emit" surface alongside the §Completion summary template. After a phase returned, the model was choosing between "dispatch next phase" and "emit a template-shaped block", and the bloat tilted the choice toward halting. Fix: folded `## Question template` into `## Between phases` (next to the gate that triggers it), trimmed `## Completion summary` rules-block bulk by ~50%, and rewrote both surfaces in strict imperative voice (MUST / NEVER / ONLY). Conductor down to 191 lines; xlfg-debug down to 180. Two existing tests updated for the new strict wording. Test surface stays at 63.

**v6.5.8** fixes a pipeline-halting regression and tightens the completion-summary shape. Regression: the bloated v6.5.5 intent-skill bullet plus the prominent v6.5.5 §Question template section together primed the conductor to halt with questions even when intent did not return `needs-user-answer`. Fix: the intent-skill bullet now states the default explicitly ("the default is to NOT halt — pick the smallest-blast-radius assumption and proceed; halting is the exception"); both conductors' §Question template sections are retitled `(used ONLY when intent returns needs-user-answer)` with an iron-clad opening sentence ("Do NOT emit questions for any other reason — uncertainty mid-pipeline, ambiguous evidence, or feeling stuck are NOT triggers"). Style: the v6.5.7 single-item collapse (`Shipped: <one clause>` on one line) is removed — every section is now `Label:\n- bullet` with a blank line before the next section, even when the section has only one bullet. Two new tests pin the shape: `test_completion_summary_label_on_own_line` and `test_completion_summary_has_blank_line_between_sections`. Test surface 61 → 63.

**v6.5.7** flips both completion-summary templates from markdown tables (v6.5.6) to bullet-driven output. Tables render as ASCII pipes in plain terminals; bullets render universally. Same labels (Shipped/Proof/Commit/Risk/Next/Archive for `/xlfg`; Mechanism/Evidence/Repair/Unknowns/Verified/Archive for `/xlfg-debug`), same ≤80-char one-clause cap, same `Files` row absence — only the layout changes. Single-item sections collapse to one line (`Shipped: <one clause>`); multi-item sections expand to bullets below the label. Tests updated to match the new shape; new `test_completion_summary_uses_bullet_format` replaces the v6.5.6 table-format test.
