...
3. Explore repo truth first with targeted reads and grep, not broad file hoarding.
4. Use specialized agents only when they buy clarity:
   - `xlfg-repo-mapper`
...
5. Write or update `context.md` with:
   - relevant repo and product context
   - hard constraints
   - known unknowns
   - likely harness / environment facts
6. Update the research and context sections of `spec.md`.
7. Create `research.md` only when external research materially changes the decision surface.
8. Keep `workboard.md` current while planning is in progress.
