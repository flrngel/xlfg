...
- Ask the user ONLY if a blocker would change correctness. "What color should the button be?" is not a blocker; "Should the webhook retry on 5xx or only on network error?" is. The default is to NOT halt — pick the smallest-blast-radius assumption, declare it in the contract, and let the conductor proceed. Halting is the exception, not the rule. If you do halt, the conductor formats the questions; you just supply each blocker as a one-clause question (≤80 chars) plus options if the answer space is finite.
