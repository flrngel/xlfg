...
> Prefer **swarm mode**: run context-expansion + planning/review agents in parallel, then execute implementation as scoped implementer/checker pairs managed by one lead. Keep ownership conflict-free and hand off only through `docs/xlfg/runs/<run-id>/`.
