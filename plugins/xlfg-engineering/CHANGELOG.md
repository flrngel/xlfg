...
- `commands/xlfg.md`: `## Completion summary` template rewritten as a markdown table — 4 mandatory rows (`Shipped`, `Proof`, `Commit`, `Archive`) and 2 optional (`Risk`, `Next` — included in the template definition but omitted at runtime when there is nothing concrete to say). The `Files` row is removed entirely. New "Hard rules" block declares the ≤80-char-per-cell cap and the no-compound-sentence / no-semicolon / no-em-dash-split / no-nested-parentheticals prohibitions, and names `see archive` as the escape hatch for content that does not fit. Variant lines (Investigation-only, Loopback escalation) shortened from two-row stubs to single-line replacements.
- `commands/xlfg.md`: `## Question template (intent halt only)` lead trimmed from `Need <n> answers before proceeding:` to `Need <n> answers:`; footer trimmed from `Blocking because:` to `Blocking:`; per-question cap added (`≤80-char question`, one clause, no compound sentences); target line tightened from `≤8 lines for 2 questions` to `≤6 lines`.
- `commands/xlfg-debug.md`: `## Completion summary` template rewritten as a markdown table with 6 mandatory rows (`Mechanism`, `Evidence`, `Repair`, `Unknowns`, `Verified`, `Archive`). The `Verified` row's value collapses from `no source edits — verified via git status --porcelain (output was empty…)` to `git status --porcelain clean`; the violation branch's `**Violated:** no-source-edits contract broken — tracked path(s): <list>` collapses to `VIOLATION: <path>` (and only the path) in the same `Verified` cell. Same Hard rules block as `/xlfg`.
- `commands/xlfg-debug.md`: `## Question template` gets the same lead/footer trim and per-question cap as `/xlfg`.
- `skills/xlfg-intent-phase/SKILL.md`: the `Ask the user ONLY if a blocker would change correctness` bullet now specifies that each blocker is a `≤80-char single-clause question` and asks the skill to "name" (not just "suggest") the options for finite-answer-space cases.
- `tests/test_xlfg_v6.py`: `test_xlfg_debug_completion_summary_item_count_matches_budget` and `test_xlfg_completion_summary_item_count_matches_budget` switch from regex `^\*\*[A-Z][^:*]+:\*\*` to a shared `_count_summary_table_rows` helper that walks the markdown table and counts data rows (skipping header / separator). Budgets stay at 6 mandatory rows. `test_conductors_carry_question_template` updates its assertions: `Need <n> answers:` (no `before proceeding`), `Blocking:` (no `because`), plus a new assertion that the `≤80` cap appears. `test_xlfg_conductor_prescribes_end_of_run_commit` updates its commit-row probe from `**Commit:**` to `| Commit `. `test_xlfg_debug_conductor_verifies_no_source_edits` drops the `verified via` assertion (the new compact `git status --porcelain clean` cell still cites the command, just without the connective) and adds a stricter assertion that the literal command appears inside the Completion-summary section, not just somewhere in the body.
- `CHANGELOG.md`, `README.md` (plugin + repo), `.claude-plugin/plugin.json`, `.cursor-plugin/plugin.json`, `NEXT_AGENT_CONTEXT.md`: version bump to 6.5.6 and test-count refresh (58 → 61).

### Added

- `test_completion_summary_uses_table_format` in `tests/test_xlfg_v6.py`: iterates both conductor bodies and asserts each `## Completion summary` section contains at least one markdown table row (`| Label | value |`). Pins the v6.5.6 format choice — future drift back toward `**Label:**` lines is loud.
- `test_completion_summary_declares_cell_length_cap`: iterates both conductor bodies and asserts each `## Completion summary` section declares the literal `≤80` substring. The cap is the load-bearing concision constraint; if the template does not name it, the model falls back to multi-clause cells and the table bloats.
- `test_xlfg_completion_summary_drops_files_row`: extracts the `## Completion summary` section of `commands/xlfg.md` and asserts the section contains neither a `| Files |` row nor a `**Files:**` row. Pins the v6.5.6 decision to drop the row — the dirty case where someone re-adds it is loud.

### Not changed

- The decision-making layer of the templates. `/xlfg`'s 4 mandatory + 2 optional row split (and which rows are which), `/xlfg-debug`'s 6 mandatory rows (and which rows are which), the `## Question template` triggering condition (`needs-user-answer` from intent), the n ≤ 3 cap on questions, the no-durable-lesson rule from v6.5.5, and the `git status --porcelain` contract from v6.5.3 are all preserved. v6.5.6 is a *content-density* patch on top of v6.5.5's *structure* patch.
- The intent skill's blocker rule, conductor pipeline order, phase count, loopback rules, end-of-run commit step, no-commit-on-debug symmetry, the sanctioned Write-path restriction, the v6.5.4 handoff-cue reminders on all 9 phase bodies, audit harness (6 checks), hooks (no `Stop`/`SubagentStop` reintroduced — fix stays prompt-only), `SANCTIONED_AGENTS`, `EXPECTED_PHASE_SKILLS`, `EXPECTED_SPECIALIST_SKILLS`, specialist skill bodies, agent `## Return format` shapes, or any phase skill's `## Done signal` text.

Patch-level because no public entry surface or runtime dependency surface changes — two terminal-output templates rewritten (table format, hard cell cap, `Files` row removed), one intent-skill bullet updated, three new regression tests, four existing tests updated to track the new strings.

## 6.5.5 — terminal-friendly question and completion-summary templates

v6.5.5 is a prompt-only patch that tightens both terminal surfaces the conductors emit to the user: the question gate that fires when the intent skill returns `needs-user-answer`, and the completion summary the conductor prints after the run lands. Pre-v6.5.5 the question instruction was a one-liner ("ask at most three numbered blocking questions and stop") with no template, so Claude tended to wrap each halt in framing prose ("To make sure I build the right thing…") that the user had to skim before reaching the actual asks. The `/xlfg` completion summary enumerated 7 mandatory items (one of them — the durable lesson — already archived in `run-summary.md`); `/xlfg-debug`'s 4–6 sentence budget routinely overflowed because items 1–4 each tended toward two or three sentences. Both surfaces hit the user every run, both bloated terminal output, and neither had a strict shape to enforce concision.

### Changed

- `commands/xlfg.md`: replaced the 7-item numbered Completion summary with a strict labeled-row template — `**Shipped:** / **Files:** / **Proof:** / **Commit:** / **Archive:**` mandatory, `**Risk:** / **Next:**` optional (omitted when empty rather than written as "none"). The durable-lesson row is dropped entirely; the lesson still lives in `run-summary.md`. Added explicit Investigation-only and Loopback-escalation variant lines so those paths land in one line each. Added a new `## Question template (intent halt only)` section between Operating contract and Loopback rules with a fenced `Need <n> answers before proceeding:` template, single-sentence-per-question rule, optional `[A) … B) …]` bracket guidance, and one `Blocking because:` footer covering all questions. The two existing references to the question gate (in `## Between phases` and the "Human-only blockers only" Operating-contract bullet) now point at the §Question template by name.
- `commands/xlfg-debug.md`: same shape-shift on both surfaces. Completion summary is now 6 labeled rows — `**Mechanism:** / **Evidence:** / **Repair surface:** / **Unknowns:** / **Verified:** / **Archive:**` — with the `**Verified:**` row carrying the load-bearing `verified via git status --porcelain` substring (the v6.5.3 contract); a `**Violated:**` replacement line is named for the contract-broken path. The "Suggested next step" footer is dropped. Added the same `## Question template (intent halt only)` section, with the only adjustment being the example opening framing ("To make sure I diagnose the right bug…" instead of "build the right thing…").
- `skills/xlfg-intent-phase/SKILL.md`: the "Ask the user ONLY if a blocker would change correctness" bullet now references the conductor's `## Question template` section explicitly and tells the intent skill to suggest finite-answer-space options where applicable so the conductor can render `[A) … B) …]` brackets and the user can reply `1A 2B` instead of typing prose.
- `tests/test_xlfg_v6.py`: `test_xlfg_debug_completion_summary_item_count_matches_budget` switches its regex from `^\s*\d+\. \*\*` (numbered items) to `^\*\*[A-Z][^:*]+:\*\*` (labeled rows) and raises the budget from ≤6 to ≤7. `test_xlfg_conductor_prescribes_end_of_run_commit` updates its commit-row assertion from `**Commit.**` (numbered-bold) to `**Commit:**` (labeled-row).
- `CHANGELOG.md`, `README.md` (plugin + repo), `.claude-plugin/plugin.json`, `.cursor-plugin/plugin.json`, `NEXT_AGENT_CONTEXT.md`: version bump to 6.5.5 and test-count refresh (55 → 58).

### Added

- `test_xlfg_completion_summary_item_count_matches_budget` in `tests/test_xlfg_v6.py`: sibling to the existing `/xlfg-debug` budget guard. Counts labeled rows (`^\*\*[A-Z][^:*]+:\*\*`) inside `## Completion summary` of `commands/xlfg.md` and asserts ≤7 mandatory rows. Pins the new terminal budget so the next drift (e.g. someone re-adding the durable-lesson row) is loud.
- `test_xlfg_completion_summary_drops_durable_lesson`: extracts the `## Completion summary` section of `commands/xlfg.md` and asserts the literal substring `durable lesson` does not appear. Companion to the budget test — explicitly forbids re-adding what v6.5.5 dropped.
- `test_conductors_carry_question_template`: iterates both conductor bodies and asserts each carries a `## Question template` section, the `Need ` lead, and the `Blocking because:` footer. Pins the question template's load-bearing substrings so future edits cannot silently drop the shape and revert to prose-style halts.

### Not changed

- The intent skill's blocker rule itself ("ask only if it would change correctness"). v6.5.5 does not change *when* questions fire, only *how they look* when they do. The cap of three blockers, the conductor's halt behavior, and the only-three-valid-mid-run-turn-boundaries rule are all unchanged.
- The compound skill's `run-summary.md` writer. The durable lesson still gets written to disk; v6.5.5 only stops repeating it in the terminal.
- Pipeline order, phase count, loopback rules, end-of-run commit step, no-commit-on-debug symmetry, the sanctioned Write-path restriction, the v6.5.4 handoff-cue reminders on all 9 phase bodies, audit harness (6 checks), hooks (no `Stop`/`SubagentStop` reintroduced — fix stays prompt-only, consistent with v6.0 philosophy), `SANCTIONED_AGENTS`, `EXPECTED_PHASE_SKILLS`, `EXPECTED_SPECIALIST_SKILLS`, specialist skill bodies, agent `## Return format` shapes, or any phase skill's `## Done signal` text.

Patch-level because no public entry surface or runtime dependency surface changes — two terminal-output templates rewritten, one intent-skill bullet updated, three new regression tests, and two existing tests updated to track the new label style.

## 6.5.4 — extend the mid-run-turn-endings guard symmetrically to all phase bodies

v6.5.4 is a prompt-only patch that closes a gap v6.5.2 left open. v6.5.2 planted a "handoff cue to the conductor, not an end-of-run marker" reminder inline on 3 of the 5 phase skills (intent, plan, implement) and deliberately skipped the 4 phase agents (recall, context, verify, review) and the 2 terminal skills (compound, debug-phase) on the rationale that the conductor's `## Between phases` section covered the agent-return case. The conductor's between-phases section is read *before* dispatch, but the decision whether to end the turn happens *after* the phase returns, inside the phase body the model just finished emitting. The 6 bodies the patch skipped all end on a structurally-terminal block — fenced `## Return format` for the 4 agents, bullet-list `## Stop-traps` for `xlfg-compound-phase`, bullet-list `## Optional specialist skills` for `xlfg-debug-phase` — with no post-amble reminder, which the model reads as a natural turn-close milestone.

### Changed

- `agents/xlfg-recall.md`, `agents/xlfg-context.md`, `agents/xlfg-verify.md`, `agents/xlfg-review.md`: each Return format now closes with a one-line reminder that the return is a handoff cue to the conductor, not an end-of-run marker, and names the conductor's next dispatch. `xlfg-recall` → intent. `xlfg-context` → next phase skill (plan for `/xlfg`, debug for `/xlfg-debug`). `xlfg-verify` → branching dispatch (review on GREEN, implement on RED, harness repair on FAILED). `xlfg-review` → branching dispatch (compound on APPROVE or APPROVE-WITH-NOTES, implement on MUST-FIX).
- `skills/xlfg-compound-phase/SKILL.md`, `skills/xlfg-debug-phase/SKILL.md`: each `## Done signal` now carries the same reminder, framed as "last *phase* but not last *step*" — compound names the conductor's end-of-run commit + completion summary as the next actions, debug-phase names the `git status --porcelain` no-source-edits check + completion-summary pointer. Neither skill writes beyond its sanctioned artifact; the reminder is prose-only.
- `CHANGELOG.md`, `README.md` (plugin + repo), `.claude-plugin/plugin.json`, `.cursor-plugin/plugin.json`, `NEXT_AGENT_CONTEXT.md`: version bump to 6.5.4 and test-count refresh (54 → 55).

### Added

- `test_phase_bodies_carry_handoff_cue` in `tests/test_xlfg_v6.py`: iterates all 9 phase bodies (5 phase skills + 4 phase agents) and asserts each contains both `handoff cue` and `not an end-of-run marker`. Pins the symmetry v6.5.2 partially introduced so the next drift is loud. Sibling to `test_conductors_forbid_mid_run_turn_endings` (which still covers the conductor-side discipline unchanged).

### Not changed

- Conductor bodies (`commands/xlfg.md`, `commands/xlfg-debug.md`). The `## Between phases` sections, `One run, one conductor turn.` rule, `do not end your turn` prohibition, and `Proceeding` anti-pattern call-out from v6.5.2 are already correct on the conductor side — the gap was on the phase-exit side, not the pre-dispatch side.
- Pipeline order, phase count, loopback rules, end-of-run commit step, no-commit-on-debug symmetry, the sanctioned Write-path restriction, audit harness (6 checks), hooks (no `Stop`/`SubagentStop` reintroduced — fix stays prompt-only, consistent with v6.0 philosophy), `SANCTIONED_AGENTS`, `EXPECTED_PHASE_SKILLS`, `EXPECTED_SPECIALIST_SKILLS`, specialist skill bodies, any `## Return format` shape or the existing trailing sentences inside each Return format.

Patch-level because no public entry surface or runtime dependency surface changes — six inline reminders appended to phase bodies and one regression test.
