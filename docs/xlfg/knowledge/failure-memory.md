...
- **Immediate fix**: v2.8.0 added a Stop hook (`phase-gate.mjs`, ported to `phase_gate.py` in v5.1.0) that reads `.xlfg/phase-state.json` and blocks if incomplete phases remain
