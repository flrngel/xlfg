...
v6.5.10 closes a phase-stopping regression that survived v6.5.8 and v6.5.9 partial fixes. Symptom: `/xlfg` halts mid-pipeline after recall / intent / plan with text like "Handing off to context phase." but no follow-up tool call. Mechanism (per Anthropic agent-loop docs): when the model emits text without a tool call, `stop_reason` becomes `end_turn` and the harness exits the loop. Cause (per diff between v6.5.2 working state and v6.5.9 broken state): two fenced "what to emit" surfaces were added v6.5.5+ that the model pattern-matched as completion targets — the `## Question template` fence (`Need <n> answers:` block, ~17 lines) and the bloated `## Completion summary` fence (13 → ~25 lines). After a phase returned, the model chose between "dispatch next phase" (described in prose) and "emit fenced template" (visible as a structural pattern); the fences won. v6.5.9 also added ALLCAPS imperative voice (NEVER / MUST / ONLY) that flattened signal-to-noise.

Fix: revert both surfaces to v6.5.2's shape. `## Between phases` is back to v6.5.2's one-liner question gate (`The intent skill returns with needs-user-answer → ask at most three numbered blocking questions and stop`) with no fenced template. `## Completion summary` is now prose-described — labels named in prose ("Mandatory sections: `Shipped:`, `Proof:`, `Commit:`, `Archive:`. Optional: `Risk:`, `Next:`."), format described inline, no fenced block. Calm imperative voice restored throughout (drop ALLCAPS NEVER/MUST/ONLY). Conductor body shrinks 191 → 163 lines for `/xlfg`; xlfg-debug 180 → 146.

The bullet-driven *output* style stays — `/xlfg` still emits `Shipped:\n- bullet\n\nProof:\n- bullet` etc. at runtime. The change is purely conductor-side: the model is told the format in prose rather than shown a fenced template it can pattern-match. The model is smart enough to follow prose description; what fails is competing fenced surfaces.

Test surface 63 → 61. Two tests removed (`test_completion_summary_label_on_own_line` and `test_completion_summary_has_blank_line_between_sections` — they pinned the v6.5.8 fenced-block shape that's been replaced by prose). `test_completion_summary_uses_bullet_format` renamed to `test_completion_summary_forbids_table_rendering` — sweeps the section for table rows without requiring a fence. `test_conductors_carry_question_template` renamed to `test_conductors_name_question_gate` — drops the `Need <n> answers:`, `Blocking:`, `≤80` assertions (pinned the removed fenced template) and asserts `needs-user-answer` (the gate trigger) and `blocking questions` (what the gate emits). `_count_summary_labels` helper replaced with `_summary_labels_referenced` (scans whole section for label references in prose).

The v6.5.4 handoff-cue reminders on all 9 phase bodies stay (well-placed; the regression was conductor-side). The v6.5.6 `Files`-row absence, v6.5.5 no-durable-lesson rule, and v6.5.3 `git status --porcelain` contract stay. Pipeline order, phase count, loopback rules, end-of-run commit step, no-commit-on-debug symmetry, sanctioned Write-path restriction, audit harness, hooks (Stop hook still NOT reintroduced — v6 prose-only philosophy holds for now), `SANCTIONED_AGENTS`, `EXPECTED_PHASE_SKILLS`, `EXPECTED_SPECIALIST_SKILLS`, specialist skill bodies, every `## Return format` shape are unchanged.

Research backing the fix (saved for the next agent who's tempted to re-add fenced templates): Anthropic agent-loop docs say the loop continues only on `stop_reason == "tool_use"`. Multi-agent prompt research (Reddit, dev.to articles, Anthropic engineering blog) consistently shows prose discipline is unreliable when competing surfaces exist. The Stop hook with `decision: "block"` is the canonical Claude Code mechanical fix per official hooks docs — but it's banned by xlfg v6 invariants and would require explicit re-architecture. v6.5.10 stays prompt-only by removing the competing surfaces, not by adding more discipline.

Version bumped 6.5.9 → 6.5.10. Patch-level — no public entry surface or runtime dependency surface changes.

## Previous state (6.5.9)

v6.5.9 fixes a phase-stopping regression that v6.5.8 only partially addressed. Symptom: the user reported runs halting after recall, intent, and plan — phases were ending the turn instead of dispatching the next phase. Diagnosis: the v6.5.5 conductor body grew 28% (162 → 208 lines) by adding a dedicated `## Question template` section. Even after v6.5.8 hardened the gate to `needs-user-answer` only, the standalone fenced template still read as a competing "what to emit" surface alongside the §Completion summary template's fenced block. After a phase returned, the model was choosing between "dispatch next phase" and "emit a template-shaped block"; the conductor bulk tilted the choice toward halting. The v6.5.4 handoff-cue reminders on all 9 phase bodies (which fixed the v6.5.2 turn-ending bug) were calibrated against a leaner conductor and got drowned out by the new bulk.

Fix: fold `## Question template` into `## Between phases` (next to the gate that triggers it), so there is one fenced surface, not two competing for "what to emit" pattern-match. The §Question template heading is gone; the template fence + gate + anti-pattern callout now live in one strict block. `## Operating contract` rewritten in strict imperative voice (MUST / NEVER / ONLY). `## Completion summary` rules-block trimmed ~50% (kept the fence, dropped the duplicative Hard rules paragraph). Conductor body shrinks from 208 → 191 lines for `/xlfg`; `/xlfg-debug` from ~205 → 180.

Two tests updated for the new strict wording. `test_conductors_forbid_mid_run_turn_endings` accepts either `do not end your turn` (legacy) or `never end the turn` (new strict phrasing). `test_conductors_carry_question_template` no longer requires a `## Question template` section heading; the template's load-bearing strings (`Need <n> answers:`, `Blocking:`, `≤80`, `needs-user-answer`) all still required, only the standalone heading went away. The `/xlfg-debug` anti-pattern transition examples were updated to include "proceeding" so the existing "Proceeding" call-out check still passes. Test surface stays at 63.

Nothing else changed. The completion-summary labels, the ≤80-char one-clause cap, the no-compound-sentences / no-semicolons / no-em-dash-splits / no-nested-parentheticals prohibitions, the `see archive` escape hatch, the `Files` section absence (v6.5.6), the no-durable-lesson rule (v6.5.5), the `Label:\n- bullet` shape with blank lines between sections (v6.5.8), the `git status --porcelain` contract (v6.5.3), the v6.5.4 handoff-cue reminders on all 9 phase bodies, and the intent-phase skill's "Ask the user ONLY if" bullet (v6.5.8 default-don't-halt lead) are all preserved. Conductor pipeline order, phase count, loopback rules, end-of-run commit step, no-commit-on-debug symmetry, the sanctioned Write-path restriction, audit harness, hooks, `SANCTIONED_AGENTS`, `EXPECTED_PHASE_SKILLS`, `EXPECTED_SPECIALIST_SKILLS`, specialist skill bodies, and every `## Return format` shape are unchanged. Version bumped 6.5.8 → 6.5.9. Patch-level — two conductor bodies trimmed and rewritten in strict imperative voice; two existing tests loosened to accept both legacy and new phrasing.

## Previous state (6.5.8)

v6.5.8 fixes a pipeline-halting regression and tightens the completion-summary shape. Two issues, addressed together. Issue 1 (pipeline regression): the user reported that runs which previously flowed through all 8 phases were now halting with question prompts mid-flow even when intent had no real blocker. Two v6.5.5 changes are the likely cause — the intent skill's "Ask the user ONLY if" bullet was bloated from one short rule into a multi-sentence directive that also explained the question-template format, and a prominent dedicated `## Question template` section was added to both conductors with a long Hard-rules block. Together these surfaces primed the conductor to halt with questions outside the `needs-user-answer` gate. Fix: the intent-skill bullet now leads with the default ("the default is to NOT halt — pick the smallest-blast-radius assumption, declare it in the contract, and let the conductor proceed; halting is the exception, not the rule") and the question-format guidance shrinks to one trailing sentence. Both conductors' `## Question template` sections are retitled `(used ONLY when intent returns needs-user-answer)`, the opening paragraph adds an iron-clad guard ("Do NOT emit questions for any other reason — uncertainty mid-pipeline, ambiguous evidence, or feeling stuck are NOT triggers"), and the Hard-rules bullet list is collapsed into one inline-rules paragraph (sections shrink from ~22 lines to ~12).

Issue 2 (completion-summary style): v6.5.7's "single-item sections collapse to one line" rule (`Shipped: <one clause>`) produced inconsistent visual rhythm in the terminal — some sections were one line, others three. The user explicitly asked for `Label:\n- bullet` everywhere with a blank line before the next section, even for single-bullet sections. Fix: both `## Completion summary` templates are rewritten so every section uses the same form — `Label:` on its own line, one or more `- bullet` lines, then a blank line before the next section. The v6.5.7 single-item collapse is removed. Optional sections (`Risk`, `Next`) keep the same shape; if there is nothing concrete to say, the section is omitted entirely (do NOT write `Risk:\n- none`). The Verified row in `/xlfg-debug` is now `Verified:\n- git status --porcelain clean`; the violation branch is `Verified:\n- VIOLATION: <path>`.

Two new regression tests pin the shape. `test_completion_summary_label_on_own_line` walks the fenced template block of both `## Completion summary` sections and asserts no `^[A-Z][A-Za-z]+:\s+\S.*$` line exists (the forbidden inline shape). `test_completion_summary_has_blank_line_between_sections` walks the same blocks and asserts no `Label:` line immediately follows a `- bullet` line — sections must be separated by at least one blank line. Test surface 61 → 63.

Nothing else changed. The labels themselves, the ≤80-char one-clause cap, the no-compound-sentences / no-semicolons / no-em-dash-splits / no-nested-parentheticals prohibitions, the `see archive` escape hatch, the `Files` section absence (v6.5.6), the no-durable-lesson rule (v6.5.5), the `git status --porcelain` contract (v6.5.3), and the v6.5.4 handoff-cue reminders on all 9 phase bodies are all preserved. Conductor pipeline order, phase count, loopback rules, end-of-run commit step, no-commit-on-debug symmetry, the sanctioned Write-path restriction, audit harness (6 checks), hooks (no `Stop`/`SubagentStop` reintroduced), `SANCTIONED_AGENTS`, `EXPECTED_PHASE_SKILLS`, `EXPECTED_SPECIALIST_SKILLS`, specialist skill bodies, and every `## Return format` shape are unchanged. Version bumped 6.5.7 → 6.5.8. Patch-level — one intent-skill bullet rewritten, two question-template sections trimmed and re-gated, two completion-summary templates rewritten, two new tests.

## Previous state (6.5.7)
...
Three changes. (1) Both completion-summary templates switch from `**Label:** value` lines to actual markdown tables (`| Label | value |`), so terminal output renders in a tight grid instead of as a flowing list. (2) Every cell carries a hard ≤80-char one-clause cap with explicit prohibitions on compound sentences, semicolons, em-dash splits, and nested parentheticals — `see archive` is the prescribed escape hatch when detail does not fit, pointing the user at `run-summary.md` / `diagnosis.md` for the full version. (3) The `Files` row is dropped entirely from `/xlfg` because `git show <sha>` and `run-summary.md` already list files; repeating them in the terminal was noise. The `/xlfg-debug` summary kept all 6 mandatory rows but the `Verified` row's value collapsed from `no source edits — verified via git status --porcelain (output was empty…)` to `git status --porcelain clean`; the violation branch collapsed correspondingly to `VIOLATION: <path>` in the same cell. The question template got matching tightening: the lead trimmed from `Need <n> answers before proceeding:` to `Need <n> answers:`, the footer from `Blocking because:` to `Blocking:`, and the same ≤80-char cap applies per question.

Three new regression tests pin the v6.5.6 shape: `test_completion_summary_uses_table_format` (both conductor summaries must contain at least one markdown table row), `test_completion_summary_declares_cell_length_cap` (both summaries must declare the literal `≤80` substring — without it the model falls back to multi-clause cells), `test_xlfg_completion_summary_drops_files_row` (the `/xlfg` summary must contain neither a `| Files |` row nor a `**Files:**` row). Four existing tests updated for the new strings: the two `*_completion_summary_item_count_matches_budget` tests switch from regex `^\*\*[A-Z][^:*]+:\*\*` to a shared `_count_summary_table_rows` helper that walks the markdown table; `test_conductors_carry_question_template` swaps `Need ` for `Need <n> answers:`, `Blocking because:` for `Blocking:`, and adds a `≤80` cap assertion; `test_xlfg_conductor_prescribes_end_of_run_commit` switches its commit-row probe from `**Commit:**` to `| Commit `; `test_xlfg_debug_conductor_verifies_no_source_edits` drops the `verified via` connective assertion (no longer needed) and adds a stricter assertion that the `git status --porcelain` literal appears inside the Completion-summary section, not just somewhere in the body. Test surface: 58 → 61.

Nothing else changed. The decision-making layer of the templates is unchanged: `/xlfg`'s 4 mandatory + 2 optional row split (Shipped / Proof / Commit / Archive mandatory; Risk / Next optional), `/xlfg-debug`'s 6 mandatory rows (Mechanism / Evidence / Repair / Unknowns / Verified / Archive), the `## Question template` triggering condition (`needs-user-answer` from intent), the n ≤ 3 cap on questions, the no-durable-lesson rule from v6.5.5, and the `git status --porcelain` contract from v6.5.3 are all preserved. v6.5.6 is a *content-density* patch on top of v6.5.5's *structure* patch. The intent skill's blocker rule, conductor pipeline order, phase count, loopback rules, end-of-run commit step, no-commit-on-debug symmetry, the sanctioned Write-path restriction, the v6.5.4 handoff-cue reminders on all 9 phase bodies, audit harness (6 checks), hooks (`Stop`/`SubagentStop` remain absent), `SANCTIONED_AGENTS`, `EXPECTED_PHASE_SKILLS`, `EXPECTED_SPECIALIST_SKILLS`, specialist skill bodies, and every `## Return format` shape are unchanged. Version bumped 6.5.5 → 6.5.6 in both manifests. Patch-level because no public entry surface or runtime dependency surface changes — two terminal-output templates rewritten, one intent-skill bullet updated, three new regression tests, four existing tests updated.

The pre-fix evidence — the bloated `/xlfg` run from a sibling project with semicolons, parentheticals, and a 7-dir `Files:` row — is in the user's chat transcript that drove this patch.

## Previous state (6.5.5)

v6.5.5 is a prompt-only patch on top of v6.5.4 that tightens both terminal surfaces the conductors emit to the user: the question gate that fires when the intent skill returns `needs-user-answer`, and the completion summary the conductor prints after the run lands. Pre-v6.5.5 the question instruction was a one-liner ("ask at most three numbered blocking questions and stop") with no template, so Claude tended to wrap each halt in framing prose ("To make sure I build the right thing…") that the user had to skim before reaching the actual asks. The `/xlfg` completion summary enumerated 7 mandatory items (one of them — the durable lesson — already archived in `run-summary.md`); `/xlfg-debug`'s 4–6 sentence budget routinely overflowed because items 1–4 each tended toward two or three sentences. Both surfaces hit the user every run, both bloated terminal output, and neither had a strict shape to enforce concision.

v6.5.5 replaces both with strict labeled-row templates. The new question template (added to both conductors as `## Question template (intent halt only)`) is `Need <n> answers before proceeding:` + numbered single-sentence questions with optional `[A) … B) …]` brackets where the answer space is finite + a single `Blocking because:` footer covering all questions. No opening framing, no per-question justification. The new `/xlfg` summary is 4 mandatory rows (`**Shipped:** / **Files:** / **Proof:** / **Commit:** / **Archive:**`) plus 2 optional ones (`**Risk:** / **Next:**` — omitted when empty rather than written as "none"); the durable-lesson row is dropped entirely (the lesson still gets written to `run-summary.md` by the compound skill, which is unchanged). The new `/xlfg-debug` summary is 6 mandatory rows — `**Mechanism:** / **Evidence:** / **Repair surface:** / **Unknowns:** / **Verified:** / **Archive:**` — with the `**Verified:**` row carrying the load-bearing `verified via git status --porcelain` substring (the v6.5.3 contract); a `**Violated:**` replacement line is named for the contract-broken path. The "Suggested next step" footer is dropped. The intent skill's blocker bullet now references the conductor's `## Question template` section explicitly and asks the skill to suggest finite-answer-space options where applicable so the conductor can render brackets and the user can reply `1A 2B` instead of typing prose.

Three new regression tests pin the templates: `test_xlfg_completion_summary_item_count_matches_budget` (sibling to the existing `/xlfg-debug` budget guard, asserts ≤7 labeled rows in `commands/xlfg.md`'s summary section), `test_xlfg_completion_summary_drops_durable_lesson` (extracts the summary section and asserts the literal `durable lesson` substring does not appear), `test_conductors_carry_question_template` (asserts both conductors carry `## Question template`, the `Need ` lead, and the `Blocking because:` footer). Two existing tests updated for the new label style: `test_xlfg_debug_completion_summary_item_count_matches_budget` switched its regex from `^\s*\d+\. \*\*` to `^\*\*[A-Z][^:*]+:\*\*` and raised the budget from ≤6 to ≤7; `test_xlfg_conductor_prescribes_end_of_run_commit` updated the commit-row assertion from `**Commit.**` to `**Commit:**`. Test surface: 55 → 58.

Nothing else changed. The intent skill's blocker rule itself ("ask only if it would change correctness") is unchanged — v6.5.5 changes *how questions look* when they fire, not *when they fire*. The cap of three blockers, the conductor's halt behavior, and the only-three-valid-mid-run-turn-boundaries rule are all unchanged. The compound skill's `run-summary.md` writer is unchanged — the durable lesson still gets archived. Pipeline order (recall → intent → context → plan → implement → verify → review → compound for `/xlfg`; recall → intent → context → debug for `/xlfg-debug`), phase count, loopback rules, `/xlfg`'s end-of-run commit step, `/xlfg-debug`'s no-source-edits check, the sanctioned Write-path restriction, the v6.5.4 handoff-cue reminders on all 9 phase bodies, audit harness (6 checks), hooks (`Stop`/`SubagentStop` remain absent — fix stays prompt-only, consistent with v6.0 philosophy), `SANCTIONED_AGENTS`, `EXPECTED_PHASE_SKILLS`, `EXPECTED_SPECIALIST_SKILLS`, specialist skill bodies, and every `## Return format` shape are unchanged. Version bumped 6.5.4 → 6.5.5 in both manifests. Patch-level because no public entry surface or runtime dependency surface changes — two terminal-output templates rewritten, one intent-skill bullet updated, three new regression tests, and two existing tests updated.

## Previous state (6.5.4)

v6.5.4 is a prompt-only patch on top of v6.5.3 that closes a mid-run turn-ending leak v6.5.2 left partially unfixed. v6.5.2 was the patch that replaced the v4-era `phase-gate.mjs` Stop hook with prompt-only discipline: it added a `## Between phases` section (plus `One run, one conductor turn.`, `do not end your turn`, and the `Proceeding` anti-pattern) to both conductor bodies, and it added a one-line inline reminder — "This is a handoff cue to the conductor, not an end-of-run marker." — to 3 of the 5 phase skills (intent, plan, implement). It **deliberately skipped** the 4 phase agents (recall, context, verify, review) and the 2 terminal phase skills (compound, debug-phase) on the stated rationale that the conductor only reads the agent's Return format block and the conductor-side between-phases section "already covers the agent-return case." That rationale was wrong. The conductor's between-phases section is read *before* dispatch, but the decision whether to end the turn happens *after* the phase returns, while the model is still finalizing its last output — and the 6 bodies v6.5.2 skipped all end on a structurally-terminal block (fenced `## Return format` for agents, `## Stop-traps` / `## Optional specialist skills` bullet lists for the terminal skills) that reads as a natural turn-close milestone. The observed symptom: `/xlfg` and `/xlfg-debug` runs "stop randomly" between phases — statistically biased toward returns from the 6 unprotected surfaces, not actually random.

v6.5.4 extends the v6.5.2 inline-reminder pattern symmetrically. The 4 phase-agent Return format sections and the 2 terminal-skill `## Done signal` sections each get one additional paragraph that (a) says "handoff cue to the conductor, not an end-of-run marker", (b) names the conductor's very next action, and (c) tells the model not to end the turn. For non-terminal phases (recall, context, verify, review) the reminder names the next dispatch — `xlfg-recall` → intent, `xlfg-context` → next phase skill (plan for `/xlfg`, debug for `/xlfg-debug`), `xlfg-verify` → review-on-GREEN / implement-on-RED / harness-repair-on-FAILED, `xlfg-review` → compound-on-APPROVE / implement-on-MUST-FIX. For the 2 terminal skills (compound for `/xlfg`, debug-phase for `/xlfg-debug`) the reminder is framed as "last *phase* but not last *step*" — compound names the end-of-run commit + completion summary as the remaining conductor steps, debug-phase names the `git status --porcelain` no-source-edits check + completion-summary pointer.

A new regression test, `test_phase_bodies_carry_handoff_cue`, iterates all 9 phase bodies (5 phase skills + 4 phase agents) and asserts each contains both `handoff cue` and `not an end-of-run marker`. It sits alongside `test_conductors_forbid_mid_run_turn_endings` (unchanged — still covers the conductor-side discipline v6.5.2 shipped). Test surface: 54 → 55.

Nothing else changed. Conductor bodies (`commands/xlfg.md`, `commands/xlfg-debug.md`) carry the v6.5.2 between-phases section unchanged — the gap was on the phase-exit side, not the pre-dispatch side. Pipeline order (recall → intent → context → plan → implement → verify → review → compound for `/xlfg`; recall → intent → context → debug for `/xlfg-debug`), phase count, loopback rules, `/xlfg`'s end-of-run commit step, `/xlfg-debug`'s no-source-edits check, the sanctioned Write-path restriction, audit harness (6 checks), hooks (`Stop`/`SubagentStop` remain absent — fix is prompt-only, consistent with v6.0 philosophy), `SANCTIONED_AGENTS`, `EXPECTED_PHASE_SKILLS`, `EXPECTED_SPECIALIST_SKILLS`, specialist skill bodies, and every `## Return format` shape are unchanged. Version bumped 6.5.3 → 6.5.4 in both manifests. Patch-level because no public entry surface or runtime dependency surface changes — six inline reminders appended to phase bodies and one regression test.

The diagnosis that identified the asymmetry is archived at `docs/xlfg/runs/20260422-235327-phase-transitions-randomly-stop/diagnosis.md` (gitignored; not tracked).

## Previous state (6.5.3)
...
v4 had a `Stop` hook (`phase-gate.mjs`) that refused to let the conductor end its turn until `.xlfg/phase-state.json.completed` listed all 8 phases. v6.0 removed that hook in the philosophy cut (commit 8784acd), replacing it with prose discipline under the premise Opus 4.7 would self-pace across phase boundaries. The premise was half-right — Opus 4.7 does hold a run in context, but the prose did not explicitly forbid mid-run turn endings, and phase done signals / return formats pattern-match as valid turn-close moments.

v6.5.2 plugs the gap with prose, not hooks (consistent with v6.0 philosophy). `commands/xlfg.md` and `commands/xlfg-debug.md` each gain a `## Between phases` section stating: (a) after any phase returns, the next tool call is the next dispatch; (b) valid mid-run turn boundaries are exactly three (`needs-user-answer` from intent, loopback cap hit, final phase returned); (c) the "Proceeding to implement" / "Moving to review" anti-pattern is called out by name. `## Operating contract`'s first bullet is now `**One run, one conductor turn.**`. The three in-`/xlfg` phase skills that are not the last one (intent, plan, implement) each gain a one-line reminder at the end of their `## Done signal` that the done signal is a handoff cue, not an end-of-run marker. Skills were touched because skill bodies load into the conductor's own context when the skill fires — the reinforcement is visible at the exact moment the conductor is deciding whether to end the turn. Agents were not touched because they run in their own sub-contexts and the conductor only reads their `## Return format` block; the conductor-side `## Between phases` section covers the agent-return case.

A new regression test, `test_conductors_forbid_mid_run_turn_endings`, asserts both conductors carry a `Between phases` section, the `one run, one conductor turn` rule, an explicit `do not end your turn` prohibition, and the `Proceeding` anti-pattern call-out. Test surface: 49 → 50 tests.
